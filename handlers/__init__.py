"""
Регистрация всех роутеров handlers.
"""

from __future__ import annotations

from aiogram import Router

from handlers.admin import router as admin_router
from handlers.appss import router as appss_router
from handlers.daily import router as daily_router
from handlers.dictionary import router as dictionary_router
from handlers.middleware import AppContextMiddleware, DatabaseMiddleware, EarlyCallbackAckMiddleware
from handlers.premium import router as premium_router
from handlers.quiz import router as quiz_router
from handlers.settings import router as settings_router
from handlers.start import router as start_router
from handlers.stats import router as stats_router


_middleware_attached = False


def get_all_routers() -> list[Router]:
    """Возвращает список роутеров с подключённым middleware."""
    global _middleware_attached
    routers = [
        appss_router,
        admin_router,
        start_router,
        daily_router,
        stats_router,
        settings_router,
        quiz_router,
        dictionary_router,
        premium_router,
    ]

    if not _middleware_attached:
        early_ack = EarlyCallbackAckMiddleware()
        db_middleware = DatabaseMiddleware()
        ctx_middleware = AppContextMiddleware()
        for r in routers:
            if r.name == "admin":
                continue
            for middleware in (early_ack, ctx_middleware, db_middleware):
                r.message.middleware(middleware)
                r.callback_query.middleware(middleware)
                r.pre_checkout_query.middleware(middleware)
        _middleware_attached = True

    return routers
